...
Process::Process (void)
  : opt_install (0),
    ...
    opt_startup (0)
{
  ACE_OS::strcpy (progname, "OemoImapMessageServer");
  ACE::init ();
}
...
ACE_NT_SERVICE_DEFINE (OemoMessageServer,
                       OemoMessageServerService,
                       ACE_TEXT ("Oemo Imap Message Server"));

int
Process::run (int argc, ACE_TCHAR* argv[])
{
  SERVICE::instance ()->name (ACE_TEXT ("OemoImapMessageServer"),
                              ACE_TEXT ("Oemo Imap Message Server"));
  ...
  if (opt_debug)
  ...
  else
    {
      ...
      ACE_NT_SERVICE_RUN (OemoMessageServer,
                          SERVICE::instance (),
                          ret);
      if (ret == 0)
        ACE_ERROR ((LM_ERROR,
